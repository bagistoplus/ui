...
const INPUT_MODES = ["text", "tel", "numeric", "decimal"] as const;
 ...
 */
export class UINumberInput extends ZagRootElement<numberInput.Props, numberInput.Api> {
  ...
  protected machineProps(): numberInput.Props {
    return {
      ...
      ids: { root: this.authoredId(), ...this.authoredIds() } as numberInput.Props["ids"],
  ...
  #inputMode(): numberInput.InputMode | undefined {
    const value = this.getAttribute("input-mode");

    return INPUT_MODES.find((mode) => mode === value);
  }

  #translations(): numberInput.IntlTranslations | undefined {
