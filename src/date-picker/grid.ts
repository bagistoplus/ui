 ...
 */
abstract class GridPart extends ZagPart<datePicker.Api, UIDatePickerTable> {
...
abstract class TextPart extends GridPart {
  #text: Text | undefined;
...
export class UIDatePickerTableHead extends GridPart {
  protected propsFor(api: datePicker.Api, owner: UIDatePickerTable): Props {
    return api.getTableHeadProps({ view: owner.view }) as Props;
  }
}

export class UIDatePickerTableBody extends GridPart {
  protected propsFor(api: datePicker.Api, owner: UIDatePickerTable): Props {
    return api.getTableBodyProps({ view: owner.view }) as Props;
  }
}

export class UIDatePickerTableRow extends GridPart {
  protected propsFor(api: datePicker.Api, owner: UIDatePickerTable): Props {
    return api.getTableRowProps({ view: owner.view }) as Props;
  }
}
...
export class UIDatePickerTableHeader extends TextPart {
  static readonly observedAttributes = ["index", "week"];

  protected propsFor(api: datePicker.Api, owner: UIDatePickerTable): Props | null {
    const view = owner.view;

    if (this.hasAttribute("week")) {
      return api.getWeekNumberHeaderCellProps({ view }) as Props;
    }

    const weekDay = this.#weekDay(api);

    return weekDay ? { ...(api.getTableHeaderProps({ view }) as Props), "aria-label": weekDay.long } : null;
  }
  ...
  #weekDay(api: datePicker.Api): datePicker.WeekDay | undefined {
    return api.weekDays[numberAttribute(this, "index") ?? -1];
  }
}

type CellArgs =
  | { view: "day"; value: datePicker.DateValue }
  | { view: "month" | "year"; props: datePicker.TableCellProps }
  | { view: "week"; weekIndex: number; week: datePicker.DateValue[] };

/** What the cell getters take, from the attributes the table wrote. */
function cellArgs(part: Element, api: datePicker.Api, owner: UIDatePickerTable): CellArgs | null {
...
export class UIDatePickerTableCell extends TextPart {
  static readonly observedAttributes = ["value", "disabled", "week-index"];
  ...
  protected propsFor(api: datePicker.Api, owner: UIDatePickerTable): Props | null {
    ...
    switch (args.view) {
      case "day":
        return api.getDayTableCellProps({ value: args.value }) as Props;
      case "month":
        return api.getMonthTableCellProps(args.props) as Props;
      case "year":
        return api.getYearTableCellProps(args.props) as Props;
      case "week":
        return api.getWeekNumberCellProps({ weekIndex: args.weekIndex, week: args.week }) as Props;
    }
  }
...
export class UIDatePickerTableCellTrigger extends TextPart {
  static readonly observedAttributes = ["value", "label", "disabled"];
  ...
  protected propsFor(api: datePicker.Api, owner: UIDatePickerTable): Props | null {
    ...
    switch (args.view) {
      case "day":
        return api.getDayTableCellTriggerProps({ value: args.value }) as Props;
      case "month":
        return api.getMonthTableCellTriggerProps(args.props) as Props;
      case "year":
        return api.getYearTableCellTriggerProps(args.props) as Props;
      case "week":
        return null;
    }
  }
