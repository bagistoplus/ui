...
import type { DateValue, SelectionMode } from "@zag-js/date-picker";
...
const SELECTION_MODES: readonly SelectionMode[] = ["single", "multiple", "range"];
 ...
 */
export class UIDatePicker extends ZagRootElement<datePicker.Props, datePicker.Api> {
  ...
  protected machineProps(): datePicker.Props {
    return {
      ...
      ids: { root: this.authoredId(), ...this.authoredIds() } as datePicker.Props["ids"],
  ...
  #view(name: string): datePicker.DateView | undefined {
   ...
   */
  #translations(): datePicker.IntlTranslations | undefined {
    const text = (name: string) => this.getAttribute(`translations-${name}`);
  ...
  #unavailableFromLists(): Unavailable | undefined {
