...
type Slot = "table" | "row" | "header" | "cell";
...
interface CellRecord {
  part: UIDatePickerTableCell;
  trigger: UIDatePickerTableCellTrigger | undefined;
}

interface RowRecord<TCell> {
  part: UIDatePickerTableRow;
  cells: TCell[];
}
 ...
 */
export class UIDatePickerTable extends ZagPart<datePicker.Api, UIDatePicker> {
  static readonly observedAttributes = ["columns", "format"];

  #table: HTMLTableElement | undefined;
  #head: UIDatePickerTableHead | undefined;
  #body: UIDatePickerTableBody | undefined;
  #header: RowRecord<UIDatePickerTableHeader> | undefined;
  #rows: RowRecord<CellRecord>[] = [];
  ...
  override render(api: datePicker.Api): void {
    ...
    applyProps(table, api.getTableProps({ view, columns: this.columns }) as Props, this.scopeFor(owner));
    this.#sync(api, view);
   ...
   */
  #clone(slot: Slot): Element | undefined {
    const template = this.querySelector<HTMLTemplateElement>(`:scope > template[data-slot="${slot}"]`);
  ...
  #grid(api: datePicker.Api, view: datePicker.DateView): Grid {
    if (view === "day") {
      const weekNumbers = api.showWeekNumbers;
      const header: HeaderSpec[] = api.weekDays.map((_, index) => ({ index }));
      const rows = api.weeks.map((week, weekIndex) => {
        const cells: CellSpec[] = week.map((date) => ({ value: date.toString() }));
    ...
    return {
      rows: grid.map((row) =>
        row.map((cell) => ({ value: String(cell.value), label: cell.label, disabled: cell.disabled })),
      ),
    };
  }

  #sync(api: datePicker.Api, view: datePicker.DateView): void {
    const table = this.#table!;
  ...
  #stampRow<TCell>(parent: Element): RowRecord<TCell> {
  ...
  #syncHeaders(record: RowRecord<UIDatePickerTableHeader>, specs: HeaderSpec[]): void {
  ...
  #syncCells(record: RowRecord<CellRecord>, specs: CellSpec[]): void {
    const tr = record.part.firstElementChild!;
   ...
   */
  #stampCell(week: boolean): CellRecord {
    const td = this.#clone("cell") ?? document.createElement("td");
    let trigger: UIDatePickerTableCellTrigger | undefined;
...
function wrap<T extends HTMLElement>(part: T, child: Element): T {
...
function setAttribute(el: Element, name: string, value: string | undefined): void {
  if (value === undefined) {
