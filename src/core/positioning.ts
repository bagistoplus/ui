import type { Placement, PositioningOptions } from "@zag-js/popover";
 ...
 */
export function readPositioning(el: Element): PositioningOptions {
  const strategy = el.getAttribute("positioning-strategy");

  return {
    placement: (el.getAttribute("positioning-placement") as Placement | null) ?? undefined,
...
function readFlip(el: Element): boolean | Placement[] | undefined {
  ...
  return listAttribute(value) as Placement[] | undefined;
}
